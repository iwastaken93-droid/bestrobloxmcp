# BestRobloxMCP Plugin Installation Guide

Complete your AI assistant integration with this easy-to-install Studio plugin. Works with Claude Code, Claude Desktop, Cursor, and any MCP-compatible AI.
...
1. **Download the plugin:**
   - **GitHub Release**: [Download MCPPlugin.rbxmx](https://github.com/bestrobloxmcp/bestrobloxmcp/releases/latest/download/MCPPlugin.rbxmx)
   - **CLI installer**: `npx -y @bestrobloxmcp/bestrobloxmcp@latest --install-plugin`
...
2. **Install to plugins folder:**
   ...
   - Keep only one MCP variant in this folder. Remove `MCPInspectorPlugin.rbxmx` if it exists from a previous install.

3. **Restart Roblox Studio** - Plugin appears automatically!

### Method 3: Save as Local Plugin
1. **Copy the plugin code:**
   - Open [studio-plugin/src/server/index.server.ts](https://github.com/bestrobloxmcp/bestrobloxmcp/blob/main/studio-plugin/src/server/index.server.ts) on GitHub (or build locally - see project README)
...
**Available Tools:** 75+ specialized tools for file trees, scripts, properties, attributes, tags, and more!
