...
function getSingleInstanceProperties(instance: Instance, excludeSource: boolean): Record<string, unknown> {
...
function getInstanceProperties(requestData: Record<string, unknown>) {
	...
	if (paths && typeIs(paths, "table") && paths.size() > 0) {
		if (!propertyName) return { error: "propertyName is required when using paths" };
	...
	const [success, result] = pcall(() => getSingleInstanceProperties(instance, excludeSource));

	if (success) {
		return { instancePath, className: instance.ClassName, properties: result };
	} else {
