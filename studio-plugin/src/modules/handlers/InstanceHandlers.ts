...
function createSingleObject(objData: Record<string, unknown>): ProcessedCreateResult {
	const className = objData.className as string;
	const parentPath = objData.parent as string;
...
function createObject(requestData: Record<string, unknown>) {
	...
	if (objects && typeIs(objects, "table") && objects.size() > 0) {
		const recordingId = beginRecording("Batch create objects");
		const { results, successCount, failureCount } = processObjectEntries(objects, createSingleObject);
		finishRecording(recordingId, successCount > 0);
		return { results, summary: { total: objects.size(), succeeded: successCount, failed: failureCount } };
	}
...
function smartDuplicate(requestData: Record<string, unknown>) {
	...
	if (duplications && typeIs(duplications, "table") && duplications.size() > 0) {
		const allResults: Record<string, unknown>[] = [];
		let totalSuccess = 0;
		let totalFailures = 0;
		const recordingId = beginRecording("Batch duplicate operations");
