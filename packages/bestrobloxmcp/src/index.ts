import { RobloxStudioMCPServer, getAllTools, getReadOnlyTools } from '@bestrobloxmcp/core';
...
if (process.argv.includes('--install-plugin')) {
  ...
  const useSafeMode = process.argv.includes('--safe-mode');
  const server = new RobloxStudioMCPServer({
    name: 'bestrobloxmcp',
    version: VERSION,
    tools: useSafeMode ? getReadOnlyTools() : getAllTools(),
  });
