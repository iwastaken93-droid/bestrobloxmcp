...
import { getPluginsFolder, handleVariantConflict } from '@bestrobloxmcp/core';

const REPO = 'bestrobloxmcp/bestrobloxmcp';
...
function httpsGet(url: string): Promise<IncomingMessage> {
  return new Promise((resolve, reject) => {
    const req = get(url, { headers: { 'User-Agent': 'bestrobloxmcp' } }, resolve);
    req.on('error', reject);
