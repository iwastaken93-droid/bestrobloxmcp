...
// Structured error codes for tool-level validation and execution errors.
// These are surfaced to the LLM so it can recover programmatically.
export type StudioToolErrorCode =
  | 'invalid_argument'
...
// Thrown by tools for validation or execution errors. Caught at the MCP
// transport layer and surfaced as a structured error with a machine-readable
// code so the LLM can recover (e.g. fix an argument) without guessing.
export interface StudioToolError {
  code: StudioToolErrorCode;
  message: string;
  data?: Record<string, unknown>;
}

export class StudioToolFailure extends Error {
  readonly toolError: StudioToolError;
  constructor(toolError: StudioToolError) {
    super(toolError.message);
    this.name = 'StudioToolFailure';
