...
import { BridgeService, RoutingFailure, StudioToolFailure, toPublic } from './bridge-service.js';
...
export const TOOL_HANDLERS: Record<string, ToolHandler> = {
  ...
  get_instance_properties: (tools, body) => tools.getInstanceProperties(body.instancePath, body.excludeSource, body.paths, body.propertyName, body.instance_id),
  ...
  set_property: (tools, body) => tools.setProperty(body.instancePath, body.propertyName, body.propertyValue, body.paths, body.instance_id),
  set_properties: (tools, body) => tools.setProperties(body.instancePath, body.properties, body.instance_id),
  create_object: (tools, body) => tools.createObject(body.className, body.parent, body.name, body.properties, body.objects, body.instance_id),
  delete_object: (tools, body) => tools.deleteObject(body.instancePath, body.instance_id),
  smart_duplicate: (tools, body) => tools.smartDuplicate(body.instancePath, body.count, body.options, body.duplications, body.instance_id),
  ...
  manage_terrain: (tools, body) => tools.manageTerrain(body.action, body.options, body.instance_id),
  spatial_query: (tools, body) => tools.spatialQuery(body.queryType, body.params, body.instance_id),
  manage_lighting: (tools, body) => tools.manageLighting(body.action, body.settings, body.instance_id),
  ...
  manage_batch: async (tools, body) => {
    const operations = body.operations as Array<{ tool: string; arguments: Record<string, unknown> }>;
    const continueOnError = body.continueOnError === true;
    const defaultInstanceId = body.instance_id as string | undefined;
    const results: Array<{ tool: string; success: boolean; result?: unknown; error?: string }> = [];
    const errors: string[] = [];

    // Tools that cannot be batched (return images, require fanout, or are unsafe)
    const UNSUPPORTED_BATCH_TOOLS = new Set([
      'manage_batch',
      'capture_screenshot',
    ...
    for (const op of operations) {
      ...
      const args = { ...op.arguments, instance_id: (op.arguments.instance_id as string | undefined) ?? defaultInstanceId };

      if (UNSUPPORTED_BATCH_TOOLS.has(toolName)) {
        const err = `Tool "${toolName}" is not supported in batch operations.`;
        results.push({ tool: toolName, success: false, error: err });
        errors.push(`${toolName}: ${err}`);
      ...
      try {
        ...
        results.push({ tool: toolName, success: false, error: message });
    ...
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ results, errors, rolledBack: !continueOnError && errors.length > 0 }),
      }],
    };
  },
};
      ...
      try {
        ...
        server.setRequestHandler(CallToolRequestSchema, async (request) => {
          ...
          try {
            ...
            if (error instanceof StudioToolFailure) {
              return {
                content: [{
                  type: 'text',
                  text: JSON.stringify({
                    error: error.toolError.code,
                    message: error.toolError.message,
                    data: error.toolError.data,
                  }),
                }],
                isError: true,
              };
