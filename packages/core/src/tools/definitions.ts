...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  // === Property Write ===
  {
    name: 'set_property',
    category: 'write',
    description: 'Set a property on an instance, or batch-set the same property on multiple instances. Pass instancePath for single, or paths array for batch.',
    inputSchema: {
      type: 'object',
      anyOf: [
      ...
      properties: {
        instancePath: {
          type: 'string',
          description: 'Instance path (dot notation). Use this for single-instance mode.'
        },
        paths: {
          type: 'array',
          items: { type: 'string' },
          description: 'Instance paths for batch mode. Use this to set the same property on multiple instances.'
        },
        propertyName: {
          type: 'string',
          description: 'Property name'
  ...
  },
  {
    ...
    category: 'read',
    description: 'Get all properties of a single instance, or get a specific property from multiple instances. Pass instancePath for single-instance full properties, or paths + propertyName for batch specific property retrieval.',
    inputSchema: {
      type: 'object',
      anyOf: [
        { required: ['instancePath'] },
      ...
      properties: {
        instancePath: {
          type: 'string',
          description: 'Instance path (dot notation). For single-instance mode; returns all properties.'
        },
        paths: {
          type: 'array',
          items: { type: 'string' },
          description: 'Instance paths for batch mode. Returns only the specified propertyName for each.'
        },
        propertyName: {
          type: 'string',
          description: 'Property name for batch mode. Only used when paths is provided.'
        },
        excludeSource: {
          type: 'boolean',
          description: 'For single-instance script mode, return SourceLength/LineCount instead of full source (default: false)'
        },
        instance_id: {
          type: 'string',
          description: 'Which connected Studio place to target. Required when multiple places are connected; omit when one. Use get_connected_instances to list available IDs.'
  ...
  // === Object Creation/Deletion ===
  {
    name: 'create_object',
    category: 'write',
    description: 'Create a new instance or multiple instances. Pass className + parent for single, or objects array for batch.',
    inputSchema: {
      type: 'object',
      anyOf: [
      ...
      properties: {
        className: {
          type: 'string',
          description: 'Roblox class name (single-instance mode)'
        },
        parent: {
          type: 'string',
          description: 'Parent instance path (single-instance mode)'
        },
        name: {
          type: 'string',
          description: 'Optional name (single-instance mode)'
        },
        properties: {
          type: 'object',
          description: 'Properties to set on creation (single-instance mode)'
        },
        objects: {
          type: 'array',
          items: {
          ...
          description: 'Objects to create for batch mode. Each entry needs className and parent.'
        },
        instance_id: {
          type: 'string',
          description: 'Which connected Studio place to target. Required when multiple places are connected; omit when one. Use get_connected_instances to list available IDs.'
  ...
  // === Duplication ===
  {
    ...
    category: 'write',
    description: 'Duplicate with naming, positioning, and property variations. Pass instancePath + count for single, or duplications array for batch.',
    inputSchema: {
      type: 'object',
      anyOf: [
      ...
      properties: {
        instancePath: {
          type: 'string',
          description: 'Instance path (dot notation) for single-duplicate mode'
        },
        count: {
          type: 'number',
          description: 'Number of duplicates for single-duplicate mode'
        },
        options: {
          type: 'object',
          properties: {
        ...
        duplications: {
          ...
          description: 'Batch duplication operations. Each entry is a smart_duplicate call.'
        },
        instance_id: {
          type: 'string',
          description: 'Which connected Studio place to target. Required when multiple places are connected; omit when one. Use get_connected_instances to list available IDs.'
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        operations: {
          ...
          items: {
            ...
            properties: {
              tool: {
                type: 'string',
                description: 'Tool name to invoke (e.g. "set_property", "create_object")'
              },
              arguments: {
                type: 'object',
                description: 'Arguments passed to the tool. instance_id is inherited from the batch if omitted.'
              }
            },
        ...
        continueOnError: {
          type: 'boolean',
          description: 'Continue executing remaining operations after a failure (default: false).'
        },
        instance_id: {
          type: 'string',
          description: 'Default instance_id for all operations in the batch. Individual operations can override via arguments.instance_id.'
        }
      },
      required: ['operations']
    }
