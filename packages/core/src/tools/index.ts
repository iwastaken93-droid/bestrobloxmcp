...
import { BridgeService, RoutingFailure, StudioToolFailure } from '../bridge-service.js';
...
export class RobloxStudioTools {
  ...
  async getInstanceProperties(instancePath: string | undefined, excludeSource: boolean | undefined, paths: string[] | undefined, propertyName: string | undefined, instance_id?: string) {
    if (paths && paths.length > 0) {
      if (!propertyName) {
        throw new Error('propertyName is required when using paths for get_instance_properties');
      }
      const response = await this._callSingle('/api/instance-properties', { paths, propertyName }, undefined, instance_id);
      return {
        content: [
          {
            type: 'text',
  ...
  async setProperty(instancePath: string | undefined, propertyName: string | undefined, propertyValue: any, paths: string[] | undefined, instance_id?: string) {
    if (paths && paths.length > 0) {
      if (!propertyName) {
        throw new Error('propertyName is required when using paths for set_property');
      }
  ...
  async createObject(className: string | undefined, parent: string | undefined, name: string | undefined, properties: Record<string, any> | undefined, objects: Array<{className: string, parent: string, name?: string, properties?: Record<string, any>}> | undefined, instance_id?: string) {
    if (objects && objects.length > 0) {
      const response = await this._callSingle('/api/create-object', { objects }, undefined, instance_id);
      return {
        content: [
          {
            type: 'text',
  ...
  async smartDuplicate(
    ...
    if (duplications && duplications.length > 0) {
      const response = await this._callSingle('/api/smart-duplicate', { duplications }, undefined, instance_id);
      return {
        content: [
          {
            type: 'text',
  ...
  private static findLibraryPath(): string {
    ...
    const overridePath = process.env.BESTROBLOXMCP_BUILD_LIBRARY || process.env.BUILD_LIBRARY_PATH;
    const cwd = path.resolve(process.cwd());
    const projectRoot = RobloxStudioTools.findProjectRoot(cwd);
    const homeLibraryPath = path.join(os.homedir(), '.bestrobloxmcp', 'build-library');
