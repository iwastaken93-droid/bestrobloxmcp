...
const VARIANTS = {
  main: {
    packageName: '@bestrobloxmcp/bestrobloxmcp',
    workspace: 'packages/bestrobloxmcp',
...
function createPlaceFixture(pluginsDir) {
  const fixtureDir = path.join(path.dirname(pluginsDir), 'bestrobloxmcp-e2e');
...
async function assertToolSurface(client, artifact, instanceId) {
  ...
  assert(names.has('execute_luau'), 'main exposes execute_luau');
  assert(names.has('set_property'), 'main exposes write tools');
...
async function main() {
  ...
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), 'bestrobloxmcp-e2e-'));
  ...
  try {
    await closeAllStudio();
    const artifact = await selectArtifact(VARIANTS.main, tmpRoot);
