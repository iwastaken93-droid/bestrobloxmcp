...
// Single unified plugin variant (no separate inspector — safe_mode is a runtime toggle).
const VARIANT = {
...
const otherVariantOutputName = 'MCPInspectorPlugin.rbxmx';
...
function injectVersion(source) {
  return source
    ...
    .replace(/__PLUGIN_VARIANT__/g, variant.scriptName)
...
if (pluginsDir) {
  ...
  const otherInstallPath = join(pluginsDir, otherVariantOutputName);
  if (existsSync(otherInstallPath)) {
    unlinkSync(otherInstallPath);
    console.log(`Removed conflicting ${otherVariantOutputName} from ${pluginsDir}`);
  }
